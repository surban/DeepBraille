...
gpa = gp_set('cf', {cfs}, 'jitterSigma2', 1e-5, 'derivobs', 'on');

%% 1) sexp
subplot(3,1,1); hold on;
opt=optimset('TolFun',1e-3,'TolX',1e-3,'DerivativeCheck','on');
%gpiaa=gp_ia(gpa, x, y);
%gpiaa=gp_optim(gpa, x, y2, 'opt',opt);
gpiaa=gpa;

[~,Varft]=gp_pred(gpiaa, x, y2, xpred);
[Eft,Covft]=gp_jpred(gpiaa, x, y2, xpred);
Eft=Eft(1:length(xpred));
Varft=Varft(1:length(xpred));
Covft=Covft(1:length(xpred), 1:length(xpred));
